...
###################################################################################################################
fishData$Weight_sqrt = sqrt(fishData$Weight)

hist(fishData$Weight, col='steelblue', main='Original')
hist(fishData$Weight_sqrt, col='coral', main='Square root Transformation')

model <- lm(formula = Weight_sqrt ~ Species + Length3 + Height + Width, data = fishData)
summary(model)
