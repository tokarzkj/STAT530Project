...
model.reduced <- lm(formula = Weight ~ Length3 + Width + Height + l3sq + wsq, data = fishData)
...
model.reduced2 <- lm(formula = Weight ~ Length3 + Width + l3sq + wsq, data = fishData)
summary(model.reduced)

anova(model.reduced2, model.reduced)
alpha = 0.05
qf(1 - alpha, 1, 153)
